...
func TestRequest_UserID(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:    "",
			request: getReq(0),
			want:    "72A5201754087B981CA83B0D565FE1FE7602DDDFF7F3FCF2B3C284BE8E93EEB2",
		}, {
			name:    "",
			request: getReq(1),
			want:    "72A5201754087B981CA83B0D565FE1FE7602DDDFF7F3FCF2B3C284BE8E93EEB2",
...
func TestRequest_ApplicationID(t *testing.T) {
	tests := []struct {
		name    string
		request *alice.Request
		want    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := tt.request
			if got := req.ApplicationID(); got != tt.want {
				t.Errorf("Request.ApplicationID() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func getReq(n int) *alice.Request {
	source := []string{`{"meta":{"client_id":"ru.yandex.searchplugin/7.16 (none none; android 4.4.2)","interfaces":{"account_linking":{},"payments":{},"screen":{}},"locale":"ru-RU","timezone":"UTC"},"request":{"command":"съешь еще этих мягких французских булок","nlu":{"entities":[],"tokens":["съешь","еще","этих","мягких","французских","булок"]},"original_utterance":"съешь еще этих мягких французских булок","type":"SimpleUtterance"},"session":{"message_id":0,"new":true,"session_id":"e19e8eee-ae065e8-36e3f907-567a814b","skill_id":"e03f8d5b-35ef-4d57-9450-b721ca17a6c3","application":{"application_id":"03B1D487CAA1C7EBF80A195491B78ACA0AC9934CDFB12A29D063A8329BC42BF0"},"user":{"user_id":"72A5201754087B981CA83B0D565FE1FE7602DDDFF7F3FCF2B3C284BE8E93EEB2"},"user_id":"03B1D487CAA1C7EBF80A195491B78ACA0AC9934CDFB12A29D063A8329BC42BF0"},"version":"1.0"}`,

		`{"meta":{"client_id":"ru.yandex.searchplugin/7.16 (none none; android 4.4.2)","interfaces":{"account_linking":{},"payments":{}},"locale":"ru-RU","timezone":"UTC"},"request":{"nlu":{"entities":[],"tokens":[]},"payload":{"msg":"ok"},"type":"ButtonPressed"},"session":{"message_id":1,"new":false,"session_id":"eeb9fa7f-940e2502-1fbf9dfb-9448a1a9","skill_id":"e03f8d5b-35ef-4d57-9450-b721ca17a6c3","user":{"user_id":"72A5201754087B981CA83B0D565FE1FE7602DDDFF7F3FCF2B3C284BE8E93EEB2"},"application":{"application_id":"03B1D487CAA1C7EBF80A195491B78ACA0AC9934CDFB12A29D063A8329BC42BF0"},"user_id":"03B1D487CAA1C7EBF80A195491B78ACA0AC9934CDFB12A29D063A8329BC42BF0"},"version":"1.0"}`,
